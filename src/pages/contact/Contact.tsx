...
const EMAILJS_SERVICE_ID  = "service_w30en7t"   // e.g. "service_abc123"
const EMAILJS_TEMPLATE_ID = "template_7x7rw4c"  // e.g. "template_xyz456"
const EMAILJS_PUBLIC_KEY  = "4Ghr7j9HG-d3COl-q"   // e.g. "aBcDeFgHiJkLmNoP"
...
export const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // EmailJS expects template variables that match your template placeholders.
    // Rename keys here to match whatever you set in your EmailJS template.
    ...
    try {
      ...
      setStatus("success")
      setFormData({ fullName: "", email: "", reason: "Full Stack Development", vision: "" })
    } catch (err: unknown) {
    ...
    <section className="bg-[#F4FAFF] relative overflow-hidden">
      ...
      <div ref={sectionRef} className="container mx-auto w-full py-14 sm:py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div ref={headerRef} className="mb-12 md:mb-20 lg:mb-24">
          ...
          <p className="text-[#5A6275] text-[15px] sm:text-[17px] md:text-[18px] leading-relaxed max-w-[672px]">
        ...
        <div ref={contentRef} className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-20">
            ...
            <div className="pt-2 sm:pt-4">
              ...
              <div className="flex gap-3">
                <button className="w-10 h-10 rounded-lg border border-[#D1D5DB] flex items-center justify-center hover:border-[#006A71] hover:bg-[#E0F2F1] transition-all duration-300">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.013 12.013 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>
                </button>
                <button className="w-10 h-10 rounded-lg border border-[#D1D5DB] flex items-center justify-center hover:border-[#006A71] hover:bg-[#E0F2F1] transition-all duration-300">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2s9 5 20 5a9.5 9.5 0 00-9-5.5c4.75 2.25 7-7 7-7"/></svg>
                </button>
                <button className="w-10 h-10 rounded-lg border border-[#D1D5DB] flex items-center justify-center hover:border-[#006A71] hover:bg-[#E0F2F1] transition-all duration-300">
          ...
          >
            ...
            {/* ── Success banner ── */}
            {status === "success" && (
              <div className="mb-5 flex items-start gap-3 rounded-[12px] bg-[#E0F2F1] border border-[#006A71]/30 px-4 py-3">
                <svg className="mt-0.5 flex-shrink-0" width="18" height="18" viewBox="0 0 24 24" fill="none">
                  <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" stroke="#006A71" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  <polyline points="22 4 12 14.01 9 11.01" stroke="#006A71" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <p className="text-[13px] font-semibold text-[#006A71]">
            ...
            {/* ── Error banner ── */}
            {status === "error" && (
              <div className="mb-5 flex items-start gap-3 rounded-[12px] bg-red-50 border border-red-200 px-4 py-3">
                <svg className="mt-0.5 flex-shrink-0" width="18" height="18" viewBox="0 0 24 24" fill="none">
            ...
            <div className="space-y-5 sm:space-y-6">
              ...
              <button
                ...
                className="w-full py-3 px-6 bg-gradient-to-r from-[#006A71] to-[#8E33E4] text-white font-bold rounded-[20px] hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 text-[14px] sm:text-[15px] disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === "sending" ? (
                  <>
