...
// icon can be a JSX.Element (default fallbacks) or a string URL/svg from the API
...
export const Skills = () => {
  ...
  const devopsCardRef = useRef<HTMLDivElement>(null);
  const progressBarsRef = useRef<(HTMLDivElement | null)[]>([]);
  const devopsTagsRef = useRef<(HTMLDivElement | null)[]>([]);
  const statsRef = useRef<(HTMLDivElement | null)[]>([]);

  const [stats, setStats] = useState<{ label: string; val: string | number }[]>([]);
  const [languagesState, setLanguagesState] = useState<
    { name: string; pct: number; color: string; sub: string }[]
  >([]);
  const [frameworksState, setFrameworksState] = useState<
    { name: string; color: string; bars: number }[]
  >([]);
  ...
  useEffect(() => {
    if (skillsLoading) return; // skeleton is still showing — DOM targets don't exist yet
    ...
    return () => ctx.revert();
  }, [skillsLoading]); // re-run once loading flips to false
      ...
      <div className="container px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-2.5">
              {frameworksState.map((fw, index) => (
                <div key={fw.name} className="border border-slate-200 rounded-[16px] p-3 sm:p-4 flex flex-col gap-2 transition-shadow duration-200 hover:shadow-md">
                  <div className="flex gap-2">{icons[index % icons.length]}</div>
              ...
              <div key={db.name} className="flex items-center gap-3 sm:gap-4 mb-5 sm:mb-6 last:mb-0">
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-[4px] gap-2">
                    ...
                    <span className={`text-[9px] font-bold tracking-widest uppercase px-1.5 py-0.5 rounded flex-shrink-0 ${db.badgeColor}`}>
                      {db.level}
                    </span>
                  </div>
                  <div className="flex gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6 lg:gap-8">
          ...
          <div ref={statsCardRef} className="bg-[#006A71] rounded-2xl p-6 sm:p-8 relative overflow-hidden">
            <svg className="absolute top-0 right-[-10px] w-28 h-24 opacity-10" viewBox="0 0 80 80" fill="none">
